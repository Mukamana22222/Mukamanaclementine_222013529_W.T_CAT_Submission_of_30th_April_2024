...
<form action="promotion.php" method="POST" onsubmit="return confirmInsert();" style="background-color: WHITE; align-items: center; width: 300px; height: 260px;"> <br><br><br>
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $stmt = $conn->prepare("INSERT INTO promotion (PromotionID,DiscountAmount,ExpirationDate) VALUES (?, ?, ?)");
    $stmt->bind_param("sss",$PromotionID, $DiscountAmount, $ExpirationDate);
...
<body>
    <center><h2>TABLE OF PROMOTION DATA</h2></center>
    ...
    <table id="dataTable">
        ...
        if ($result->num_rows > 0) {
            while ($row = $result->fetch_assoc()) {
                echo "<tr><td>" . $row["PromotionID"] .
                     "</td><td>" . $row["DiscountAmount"] .
                     "</td><td>" . $row["ExpirationDate"] .
                     "</td><td><a href='promupdate.php?updatePromotionID=". $row['PromotionID']."'>UPDATE</a></td><td><a href='promdelete.php?deletePromotionID=".$row['PromotionID']."'>DELETE</a></td></tr>";       
            }
        } else {
